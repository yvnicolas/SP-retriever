...
public class LIConnectionRetrieverImpl extends
		DynSPConnectionRetriever<LinkedInProfile> {

	private static final Logger logger = LoggerFactory
			.getLogger(LIConnectionRetrieverImpl.class);
	...
	public List<PersonBasic> getConnections() throws SpInfoRetrievingException {
		...
		for (LinkedInProfile connection : connections) {
			toReturn.add(new PersonBasic(connection.getFirstName(), connection
					.getLastName()));
		}
		return toReturn;
	}
	...
	public String getPermissions() {
		// TODO : ameliorer en retournant les permissions autorisees si
		// connectees
	...
	List<LinkedInProfile> getMatchesAsProfiles(PersonWthAddress person) {
		...
		logger.debug(String.format("Found %s Linkedin profiles matches for %s",
